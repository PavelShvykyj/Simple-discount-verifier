...
| PR-3 | Public Barcode And Support QR | review | Dynamic barcode rendering, quiet support action, pre-correlation explanation, lazy support QR dialog, and visible plain `correlationId` are implemented. | `npm --prefix frontend run lint` and `npm --prefix frontend run build` passed on 2026-06-30; build kept the existing initial bundle budget warning. Unit tests intentionally deferred until after review. |
| PR-4 | Admin Customer Profiles | todo | Implement staff customer profile list, lookup, create, detail, and edit flows. | |
